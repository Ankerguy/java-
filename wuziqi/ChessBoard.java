...
public class ChessBoard extends JPanel implements MouseListener{
	...
	public ChessBoard(){
		...
		addMouseMotionListener(new MouseMotionListener() {//匿名监听
			public void mouseMoved(MouseEvent e) {//鼠标点击下棋
				...
				if(x1<0||x1>ROWS||y1<0||y1>COLS||gameOver||findChess(x1,y1)){//游戏已经结束，不能下；落在棋盘外，不能下；x，y位置已经有棋子存在，不能下
					setCursor(new Cursor(Cursor.DEFAULT_CURSOR));//光标设置成默认形状
				}else{
					setCursor(new Cursor(Cursor.HAND_CURSOR));//光标设置成手型
				}
			}
	...
	public void paintComponent(Graphics g){
		...
		for(int i=0;i<chessCount;i++){
			...
			g.fillOval(xPos-Point.DIAMETER/2, yPos-Point.DIAMETER/2, Point.DIAMETER, Point.DIAMETER);//填充⚪
			if(i==chessCount-1){
